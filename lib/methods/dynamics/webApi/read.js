...
module.exports = (
  ...
  const readEnrolmentRequests = async (serviceId, contactId, onlyUnspent = true) => {
    ...
    const response = requestPromise(request)
  ...
  readEnrolmentRequests.buildRequest = async (serviceId, contactId, onlyUnspent) => {
    const filter = [
      `defra_service eq ${serviceId}`,
      `defra_serviceuser eq ${contactId}`
    ]
  ...
  readEnrolmentRequests.parseResponse = (response) => {
    ...
    return data.value.map(enrolmentRequest => models.EnrolmentRequest.fromPlainObject({
      ...
      serviceId: enrolmentRequest.defra_service,
      accountId: enrolmentRequest.defra_organisation,
      contactId: enrolmentRequest.defra_serviceuser,
      connectionDetailsId: enrolmentRequest.defra_connectiondetail,
