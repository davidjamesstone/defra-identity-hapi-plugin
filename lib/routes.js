...
module.exports = (
  ...
  server.route({
    ...
    handler: async (request, h) => {
      try {
        ...
        const outboundUrl = await server.methods.idm.generateOutboundRedirectUrl(request, request.query, { state, nonce, scope, serviceId })
  ...
  server.route({
    ...
    handler: (request, h) => {
      ...
      const response = h.response(version).type('text/plain')
      return response
    }
  })
